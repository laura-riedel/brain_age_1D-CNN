...
class shallow1DCNN(pl.LightningModule):
    """
    1D Convolutional Neural Network (CNN) that takes (fMRI) timeseries as input and predicts
    (i.e., regresses) a participant's age.
    Explicit, non-repetitive version of the final shallow variable1DCNN that was determined 
    ...
    def __init__(self, loss=nn.MSELoss(), lr=1e-3, weight_decay=0, 
        ...
        self.encoder = nn.Sequential(
            nn.Conv1d(in_channels=100, out_channels=128, kernel_size=9),
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.Conv1d(in_channels=128, out_channels=128, kernel_size=9),
            nn.BatchNorm1d(128),
            nn.ReLU(),
